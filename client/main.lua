...
function Process(action, start, tick, finish)
    ...
    if not IsEntityDead(ped) or Action.useWhileDead then
        if not isDoingAction then
            ...
            TriggerEvent("LXCore:Notify", "You are already doing something!", "error")
        end
    else
        TriggerEvent("LXRCore:Notify", "Cant do that action!", "error")
    end
end
...
-- Example Usage using the lxrcore export:
...
--     local ped = PlayerPedId()
--     exports['lxr-core']:Progressbar("use_bandage", "Look at me mah, Im doing stuff!", 10000, false, true, {
